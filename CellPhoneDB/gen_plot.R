...
res_path = "/stanley/levin_dr_storage/kwanho/jeff_microglia/analysis_071321/cpdb/combined"
...
write.table(pval, file.path(res_path, "filtered_comb_pvalues.txt"), sep='\t', quote=F, row.names=F, col.names=T)
write.table(means, file.path(res_path, "filtered_comb_means.txt"), sep='\t', quote=F, row.names=F, col.names=T)
...
dot_plot(means_path=file.path(res_path, "filtered_comb_means.txt"), pvalues_path=file.path(res_path, "filtered_comb_pvalues.txt"), filename=file.path(res_path, "cpdb_comb_res.pdf"), height=32, width=40)



# prep final tables

pval_file = file.path(res_path, "filtered_comb_pvalues.txt")
means_file = file.path(res_path, "filtered_comb_means.txt")
pval = read.table(pval_file, sep='\t', header=T)
means = read.table(means_file, sep='\t', header=T)

my.cols = colnames(pval)[c(1:11, 42:95)]
pval = pval[,my.cols]
means = means[,my.cols]

df <- pval[,c(12:ncol(pval))]
rows.use = apply(df, 1, min) < 0.05
rows.use[which(duplicated(pval$interacting_pair))] = F
