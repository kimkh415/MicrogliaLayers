function cellmask=findallcells(mypath,genelist)
...
SE=strel('disk',2);
thisImage=bwareaopen(imdilate(thisImage,SE),600);
thisImage=thisImage-bwareaopen(imdilate(thisImage,SE),1500);
SE=strel('disk',5);
thisImage=bwareaopen(imdilate(thisImage,SE),800);
thisImage=thisImage-bwareaopen(imdilate(thisImage,SE),3000);
SE=strel('disk',6);
thisImage=imfill(bwareaopen(thisImage,800),'holes');
cellmask=thisImage;
